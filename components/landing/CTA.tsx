...
export default function CTA({ isSignedIn = false }: CTAProps) {
  return (
    <section className="container mx-auto px-4 sm:px-6 text-center pb-24 animate-fade-up">
      <h2 className="text-2xl md:text-3xl font-instrument-serif mb-6">
        {isSignedIn ? 'Ready to create your next project?' : 'Ready to level-up your screenshots?'}
      </h2>
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        {isSignedIn ? (
          <>
            ...
            >
              <Rocket className="w-5 h-5" /> New Project
            </Link>
          </>
        ) : (
          <>
